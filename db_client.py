...
class PowerPlantDBClient():
    ...
    def get_plants(self, auth_ticket, plants):
        cursor = self.db_connection.cursor()
        print(f'trying to fetch: {plants}')
        if len(plants)==0:
            cursor.execute(f'SELECT P.plant_id, type, current_production,current_consumption, stored_charge, active from prosumers AS P inner join tickets AS T on P.plant_id=T.plant_id WHERE T.token="{auth_ticket}" AND T.token_expires>DATE(NOW());')
        else:
            #Load the requested plant identifiers.
            plant_string = "T.plant_id="
            prep_stmnt = ""
            for plant_id in plants:
                prep_stmnt+= plant_string+str(plant_id)+' OR '
            prep_stmnt = prep_stmnt.rstrip(' OR ')
            cursor.execute(f'SELECT P.plant_id, type, current_production,current_consumption, stored_charge, active from prosumers AS P inner join tickets AS T on P.plant_id=T.plant_id WHERE ({prep_stmnt}) AND T.token="{auth_ticket}" AND T.token_expires>DATE(NOW());')
    ...
    def get_token_valid(self, token, requested_plants):
        cursor = self.db_connection.cursor()
        cursor.execute(f'SELECT plant_id FROM tickets WHERE token={token} AND token_expires>DATE(NOW());')
        res = cursor.fetchall()
        self.db_connection.commit()
        return res
    ...
    def get_token_expiration(self, token):
        cursor = self.db_connection.cursor()
        print(f"PERFORMING QUERY token : {token}")
        ...
        self.db_connection.commit() # Nothign to commit, just updating db to sync with others.
